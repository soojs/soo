...
const { PostEnum } = require('../../app/enum');
...
describe('test/api/post.test.js', () => {
  ...
  describe('#create', () => {
    ...
    it('should create post and return the created data', async () => {
      ...
      const post2 = await PostService.getById(tempPostId, PostEnum.Format.MARKDOWN);
      assert(post2 !== null);
  ...
  describe('#publish', () => {
    ...
    before(async () => {
      ...
      assert(created.status === PostEnum.Status.DRAFT);
      tempPostId = created.id;
    });
    after(async () => {
      await PostService.remove(tempPostId);
    ...
    it('should update post and return the updated data with status 1', async () => {
      ...
      assert(updated.status === PostEnum.Status.RELEASE);
    });
  });
});
